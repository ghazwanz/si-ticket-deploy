<x-guest-layout>
    <div class="text-center">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-3xl">Secure Area</h1>
        <p class="mt-2 text-sm text-slate-505 dark:text-slate-400">Please enter your password to continue.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.confirm') }}" class="mt-8 space-y-5">
        ...
        <div>
            <label for="password" class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Password <span class="text-red-500">*</span></label>
            <div class="relative">
        ...
        <button type="submit" class="group relative flex w-full justify-center mt-6 rounded-xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-violet-755 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 dark:focus:ring-offset-slate-950 transition-all disabled:opacity-50">
            Confirm Password
            <x-heroicon-o-shield-check class="ml-2 h-5 w-5 transition-transform group-hover:scale-110" />
        </button>
    </form>
</x-guest-layout>
